...
db.tmppn.find().addOption(DBQuery.Option.noTimeout).forEach(
    function(pn_doc) {
    	// var existing_level_3_document = db.phoneNumbers.findOne({level_3: pn_doc.level_3, level_1: pn_doc.level_1, misc: pn_doc.misc})
        // If the level 3 document does not yet exist. 
        var existing_level_3_document = db.phoneNumbers.findOne({level_3: pn_doc.level_3})
    	if (existing_level_3_document == null) {
    		db.phoneNumbers.insert({
                level_1: pn_doc.level_1,
                level_2: pn_doc.level_2,
    			level_3: pn_doc.level_3,
    			numbers: [{person: pn_doc.person, number: pn_doc.number}],
    			misc: pn_doc.misc
    		})
    	} else {
            // If the level 3 document already exists. 
    		var numbers_array = existing_level_3_document.numbers
    		var new_array_entry = {person: pn_doc.person, number: pn_doc.number}
    		numbers_array.push(new_array_entry)
    		db.phoneNumbers.update({_id: existing_level_3_document._id},
